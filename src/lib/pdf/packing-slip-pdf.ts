...
const MARGIN = 18;
const THUMB_MM = 12;
const FALLBACK_IMAGE = "/images/hub-of-craftss-logo.png";
...
export async function downloadOrderPdf(order: PackingSlipOrder) {
  ...
  forceDownloadPdf(blob, buildTimestampedFilename("Hub_Order"));
}
...
export async function downloadOrdersPdf(orders: PackingSlipOrder[]) {
  ...
  const blob = doc.output("blob");
  forceDownloadPdf(blob, buildTimestampedFilename("Hub_Orders"));
}
