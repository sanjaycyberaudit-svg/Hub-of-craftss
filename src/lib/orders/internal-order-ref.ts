 ...
 * Example: September 2026, 1st paid order → `26090001`
 *
 * Assigned only when payment becomes paid (not at checkout create).
 */

export const INTERNAL_ORDER_REF_SEQ_DIGITS = 4;
export const INTERNAL_ORDER_REF_MAX_SEQ = 10 ** INTERNAL_ORDER_REF_SEQ_DIGITS - 1; // 9999

/** IST calendar year/month for the prefix (shop timezone). */
export function internalOrderRefPrefixFromDate(
  now: Date = new Date(),
): { yymm: string; year: number; month: number } {
